...
async fn tokio_rayon_separate_pipeline() -> Duration {
    ...
    let cpu_results: Vec<usize> = data.par_iter().map(|&x| cpu_work(x % 100)).collect();
...
async fn manual_distributed_processing() -> Duration {
    ...
    println!(
        "Manual distributed processing created {} partitions",
        results.len()
    );
    start.elapsed()
}
...
async fn moirai_async_coordination() -> Duration {
    ...
    let (broadcast_tx, _broadcast_rx) = Broadcast::new(100);
    let broadcast_tx = Arc::new(broadcast_tx);

    // State monitoring
    let (watch_tx, _watch_rx) = Watch::new(0u32);
    ...
    for i in 0..20 {
        ...
        let watch_tx = Arc::clone(&watch_tx);
    ...
    println!(
        "Moirai async coordination completed {} tasks",
        results.len()
    );
    start.elapsed()
}
