...
#[derive(Debug, Clone)]
enum TaskResult {
    Success { output: Vec<u8>, execution_time_ms: u64 },
    Failure { error: String, retry_after_ms: Option<u64> },
}
...
impl ConsensusEngine {
    ...
    fn append_entry(&self, command: ConsensusCommand) -> Result<u64, String> {
        ...
        let mut log = self.log_entries.lock()
...
impl DistributedTaskScheduler {
    fn new(node_id: NodeId, max_capacity: usize) -> Result<Self, String> {
        ...
        nodes.insert(node_id.clone(), NodeState::new(node_id.clone(), max_capacity));
    ...
    fn join_cluster(&self, other_node: NodeId, max_capacity: usize) -> Result<(), String> {
        ...
        nodes.insert(other_node.clone(), NodeState::new(other_node.clone(), max_capacity));
        
        // Log the join operation
        let command = ConsensusCommand::NodeJoin { node_id: other_node };
    ...
    fn assign_task_to_node(&self, task_id: u64, node_id: NodeId) -> Result<bool, String> {
        ...
        let command = ConsensusCommand::TaskAssignment { task_id, node_id: node_id.clone() };
        self.consensus.append_entry(command)?;
        ...
        let handle = self.runtime.spawn_fn_with_priority(move || {
            ...
            match execution_result {
                Ok(output) => {
                    ...
                    // Log completion
                    let result = TaskResult::Success { output, execution_time_ms: execution_time };
                    ...
                    println!("Task {} completed on {} in {}ms", task_id, node_id, execution_time);
                }
                Err(error) => {
                    // Handle task failure
                    {
                    ...
                    let result = TaskResult::Failure { 
                        error: error.clone(), 
                        retry_after_ms: if task_data.retry_count < task_data.max_retries { Some(1000) } else { None }
                    };
                    let command = ConsensusCommand::TaskCompletion { task_id, result };
                    let _ = consensus.append_entry(command);
                    ...
                    if task_data.retry_count >= task_data.max_retries {
                        ...
                        println!("Task {} failed permanently on {}: {}", task_id, node_id, error);
                    } else {
                        println!("Task {} failed on {} (retry {}/{}): {}", 
                                task_id, node_id, task_data.retry_count + 1, task_data.max_retries, error);
                    }
                }
            }
        }, priority);
    ...
    fn simulate_network_partition(&self, isolated_nodes: Vec<NodeId>) -> Result<(), String> {
        println!("SIMULATING NETWORK PARTITION: Isolating nodes {:?}", isolated_nodes);
    ...
    fn recover_from_partition(&self, recovered_nodes: Vec<NodeId>) -> Result<(), String> {
        println!("RECOVERING FROM PARTITION: Restoring nodes {:?}", recovered_nodes);
    ...
    fn trigger_leader_election(&self) -> Result<(), String> {
        ...
        if let Some(leader_id) = new_leader {
            ...
            // Log leader election
            let command = ConsensusCommand::LeaderElection { candidate_id: leader_id.clone() };
            self.consensus.append_entry(command)?;
    ...
    fn get_cluster_status(&self) -> Result<ClusterStatus, String> {
        ...
        let pending_tasks = tasks.values().filter(|(_, status)| *status == TaskStatus::Pending).count();
        let running_tasks = tasks.values().filter(|(_, status)| *status == TaskStatus::Running).count();
...
impl SchedulerRef {
    fn schedule_tasks(&self) -> Result<usize, String> {
        ...
        let tasks_to_schedule: Vec<u64> = {
            ...
            let mut tasks = Vec::new();
            for _ in 0..available_nodes.len().min(10) { // Limit batch size
