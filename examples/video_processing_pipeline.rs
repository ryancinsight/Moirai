...
impl SIMDProcessor {
    ...
    fn denoise_frame(&self, frame: &mut VideoFrame) -> Result<(), String> {
        ...
        self.pixels_processed.fetch_add(frame.pixel_count() as u64, Ordering::Relaxed);
    ...
    fn simd_denoise_rgb24(&self, data: &mut [u8], width: usize, height: usize) -> Result<(), String> {
    ...
    fn fallback_denoise_rgb24(&self, data: &mut [u8], width: usize, height: usize) -> Result<(), String> {
    ...
    fn denoise_rgba32(&self, data: &mut [u8], width: usize, height: usize) -> Result<(), String> {
        ...
        for y in 1..height-1 {
            for x in 1..width-1 {
                for c in 0..3 { // Don't blur alpha channel
                    let mut sum = 0u32;
    ...
    fn sharpen_frame(&self, frame: &mut VideoFrame) -> Result<(), String> {
        ...
        for y in 1..frame.height-1 {
            for x in 1..frame.width-1 {
                for c in 0..channels.min(3) { // Don't sharpen alpha
                    let mut sum = 0.0f32;
    ...
    fn color_correct_frame(&self, frame: &mut VideoFrame) -> Result<(), String> {
        ...
        for chunk in frame.data.chunks_mut(channels * 16) { // Process 16 pixels at a time
            for pixel_data in chunk.chunks_mut(channels) {
                for c in 0..channels.min(3) { // Don't adjust alpha
                    let original = pixel_data[c] as f32;
                    
                    // Apply brightness and contrast
                    let adjusted = ((original - 128.0) * contrast + 128.0 + brightness).clamp(0.0, 255.0);
    ...
    fn resize_frame(&self, frame: &mut VideoFrame, new_width: usize, new_height: usize) -> Result<(), String> {
        let start_time = Instant::now();
    ...
    fn stats(&self) -> (usize, usize, usize, u64, u64, f64) {
        ...
        (total_ops, simd_ops, fallback_ops, total_time, pixels, simd_rate)
    }
}
...
impl VideoProcessingPipeline {
    fn new(preprocessor_workers: usize, effects_workers: usize, output_workers: usize) -> Result<Self, String> {
    ...
    fn start(&self) -> Result<(), String> {
        ...
        println!("Video processing pipeline started with {} preprocessor, {} effects, {} output workers", 
        ...
        let handle = self.runtime.spawn_fn_with_priority(move || {
            while is_running.load(Ordering::Relaxed) {
                ...
                let success = match Self::preprocess_frame(&mut frame, &simd_processor) {
                    ...
                    Err(e) => {
                        println!("Preprocessing worker {}: Frame {} failed: {}", worker_id, frame.id, e);
                        false
                    }
                };
    ...
    fn preprocess_frame(frame: &mut VideoFrame, simd_processor: &SIMDProcessor) -> Result<(), String> {
    ...
    fn start_effects_worker(&self, worker_id: usize) -> Result<(), String> {
        ...
        let handle = self.runtime.spawn_fn_with_priority(move || {
            while is_running.load(Ordering::Relaxed) {
                ...
                let success = Self::apply_effects(&mut frame, &simd_processor, worker_id).is_ok();
    ...
    fn apply_effects(frame: &mut VideoFrame, simd_processor: &SIMDProcessor, worker_id: usize) -> Result<(), String> {
        ...
        if worker_id == 0 && fastrand::f64() < 0.1 {
            println!("Effects worker {}: Processed frame {} (quality: {:.1})", 
    ...
    fn process_output(frame: &VideoFrame, memory_pool: &VideoMemoryPool, worker_id: usize) -> Result<(), String> {
        ...
        if worker_id == 0 && fastrand::f64() < 0.1 {
            println!("Output worker {}: Encoded frame {} ({}x{}, quality: {:.1})", 
    ...
    fn queue_sizes(&self) -> (usize, usize, usize, usize) {
        ...
        let preprocessing_size = self.preprocessing_queue.lock().map(|q| q.len()).unwrap_or(0);
    ...
    fn get_statistics(&self) -> PipelineStats {
        let (simd_ops, simd_count, fallback_count, simd_time, simd_pixels, simd_rate) = self.simd_processor.stats();
        let (mem_allocated, mem_reused, mem_current, mem_peak, mem_reuse_rate) = self.memory_pool.stats();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Generated {} test frames with various resolutions and formats", test_frames.len());
    ...
    while monitoring_start.elapsed() < Duration::from_secs(30) {
        ...
        if stats.frames_processed > last_processed {
            ...
            println!("  Progress: {}/{} frames | Queues: I:{} P:{} E:{} O:{} | SIMD: {:.1}%",
                     stats.frames_processed, 
                     stats.frames_processed + stats.frames_failed,
                     input_q, prep_q, fx_q, out_q,
    ...
    println!("  │  ├─ Success rate: {:.1}%", 
             (final_stats.frames_processed as f64 / (final_stats.frames_processed + final_stats.frames_failed).max(1) as f64) * 100.0);
    println!("  │  └─ Avg processing time: {:.1}ms per frame", 
             final_stats.total_processing_time_us as f64 / (final_stats.frames_processed.max(1) as f64 * 1000.0));
    
    println!("  ├─ SIMD Optimization:");
    println!("  │  ├─ Total operations: {}", final_stats.simd_operations);
    println!("  │  ├─ SIMD usage rate: {:.1}%", final_stats.simd_usage_rate);
    println!("  │  ├─ Pixels processed: {:.2}M", final_stats.pixels_processed as f64 / 1_000_000.0);
    println!("  │  └─ Pixel throughput: {:.1}M pixels/sec", 
             final_stats.pixels_processed as f64 / (final_stats.total_processing_time_us as f64 / 1_000_000.0));
    
    println!("  ├─ Memory Management:");
    println!("  │  ├─ Buffers allocated: {}", final_stats.memory_allocated);
    println!("  │  ├─ Buffers reused: {}", final_stats.memory_reused);
    println!("  │  ├─ Reuse rate: {:.1}%", final_stats.memory_reuse_rate);
    println!("  │  ├─ Current memory: {:.2} MB", final_stats.current_memory_bytes as f64 / 1_048_576.0);
    println!("  │  └─ Peak memory: {:.2} MB", final_stats.peak_memory_bytes as f64 / 1_048_576.0);
    
    println!("  ├─ Queue Performance:");
    println!("  │  ├─ Input queue: {}", final_stats.input_queue_size);
    println!("  │  ├─ Preprocessing queue: {}", final_stats.preprocessing_queue_size);
    println!("  │  ├─ Effects queue: {}", final_stats.effects_queue_size);
    println!("  │  ├─ Output queue: {}", final_stats.output_queue_size);
    println!("  │  └─ Avg queue wait: {:.1}ms", final_stats.queue_wait_time_us as f64 / 1000.0);
    
    println!("  └─ Throughput Analysis:");
    let total_time_sec = monitoring_start.elapsed().as_secs_f64();
    println!("     ├─ Processing rate: {:.1} frames/sec", final_stats.frames_processed as f64 / total_time_sec);
    println!("     ├─ Data throughput: {:.1} MB/sec", 
