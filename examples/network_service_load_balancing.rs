...
impl LoadBalancer {
    ...
    fn handle_request(&self, request: ServiceRequest) -> Result<(), String> {
        ...
        let pending_count = self.pending_requests.read()
    ...
    fn select_instance(&self, request: &ServiceRequest) -> Result<Arc<ServiceInstance>, String> {
        ...
        let available_instances: Vec<_> = instances.values()
        ...
        match self.algorithm {
            LoadBalancingAlgorithm::RoundRobin => {
                let index = self.round_robin_counter.fetch_add(1, Ordering::Relaxed) % available_instances.len();
                Ok(available_instances[index].clone())
            }
            LoadBalancingAlgorithm::LeastConnections => {
                let instance = available_instances.iter()
    ...
    fn route_request_to_instance(&self, request: ServiceRequest, instance: Arc<ServiceInstance>) -> Result<(), String> {
    ...
    fn simulate_request_processing(request: &ServiceRequest, instance: &ServiceInstance) -> Result<ServiceResponse, String> {
    ...
    fn get_statistics(&self) -> LoadBalancerStats {
        ...
        let total_capacity = instances.values()
            .map(|instance| instance.max_capacity)
            .sum();
        
        let current_load = instances.values()
    ...
    fn check_timeouts(&self) {
        if let Ok(mut pending) = self.pending_requests.write() {
            ...
            for (request_id, (request, start_time)) in pending.iter() {
                let timeout = Duration::from_millis(request.timeout_ms.max(self.default_timeout_ms));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1000..2000 {
        let request = ServiceRequest {
            ...
            priority: if i % 10 == 0 { RequestPriority::Critical } else { RequestPriority::Normal },
            timestamp: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs(),
            timeout_ms: 2000,
        };
    ...
    println!("     ├─ Total throughput: {:.0} req/s", 
             stats.total_requests as f64 / start_time.elapsed().as_secs_f64());
    println!("     └─ Instance efficiency: {:.1}%", 
    ...
    if let Ok(instances) = load_balancer.instances.read() {
        for (id, instance) in instances.iter() {
            ...
            println!("    ├─ Healthy: {}", instance.is_healthy.load(Ordering::Relaxed));
    ...
    for (i, algorithm) in algorithms.iter().enumerate() {
        ...
        println!("    └─ Success rate: {:.1}%, Avg time: {}ms, Duration: {:?}",
                 test_stats.success_rate(), test_stats.avg_response_time_ms, test_time);
