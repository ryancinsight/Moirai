...
fn version_artifacts_are_synchronized_for_current_target() {
    ...
    for required in [
        "version = \"0.3.1\"",
        "moirai-core = { path = \"moirai-core\", version = \"0.3.1\" }",
        "moirai-executor = { path = \"moirai-executor\", version = \"0.3.1\" }",
        "moirai-iter = { path = \"moirai-iter\", version = \"0.3.1\" }",
    ] {
        assert!(
    ...
    assert!(
        checklist.contains("**Target Version**: 0.3.1"),
        "checklist target version must match Cargo workspace version"
    );
    assert!(
        changelog.contains("## [0.3.1] - 2026-07-15"),
        "changelog must contain a synchronized 0.3.1 section"
    );
}
