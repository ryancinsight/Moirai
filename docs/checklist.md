...
- [ ] [arch] ISSUE-199: Add accelerator route topology for CPU/GPU/TPU/NPU placement metadata with sealed ZST route policies, transparent device identifiers, value-checked route-summary benchmarks, and source contracts that reject fabricated backend execution.
- [ ] [arch] ISSUE-200: Extend Mnemosyne-owned payload regions for device handoff so accelerator routes have explicit pointer-transfer rejection and owned-byte handoff semantics.
- [ ] [minor] ISSUE-201: Expose top-level fixed-capability routed execution over existing process/server clients without arbitrary closure remoting or placeholder node discovery.
