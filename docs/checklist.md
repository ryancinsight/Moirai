...
- [ ] [arch] ISSUE-197: Connect route values to transport-backed process/server executors and the Mnemosyne allocator ownership boundary; route-to-transport address consumption and bounded TCP remote byte transport are implemented, while OS process lifecycle, remote task envelopes/results, production server backpressure, and allocator handoff remain open.
