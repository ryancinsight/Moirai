...
- [ ] [minor] Rayon ecosystem parity remains bounded by the audited subset, including bounded exact-size indexed source cardinality and collect-into-vec, terminal reducers, fallible reducers, `try_reduce_with`, position terminals, `positions`, stateful side-effect terminals, reference materialization, `update`, `intersperse`, `zip_eq`, `partition_map`, and sorting; add new adapter or slice-extension surfaces only with value-semantic tests, benchmark-contract coverage, and same-run Rayon comparison rows.
