...
**Current verification**: focused 0.3.0 formatter, warning-denied all-target/all-feature Clippy, rustdoc, and nextest gates pass for `moirai-core`, `moirai-iter`, and `moirai-benchmarks` (323 tests, 2 cfg-skips). The bounded scheduler admission benchmark contract verifies fallible queue publication and explicit capacity handling. Themis and Mnemosyne follow their provider default branches; the resolved graph has one Themis source identity. The focused 0.3.1 regression gate remains in progress.
