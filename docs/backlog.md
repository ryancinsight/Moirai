...
- Residual: a raw Melinoe call before application initialization still uses the
  foundation fallback. Making that call order-insensitive would require a
  dependency-free provider-discovery hook or a platform constructor, both
  outside this contract and incompatible with the WebAssembly target. The
  measured fallback cost remains recorded in ADR 0057; a future Melinoe seam
  reopens this item.
...
- **Standalone harness (2026-09-17).** The bounded
  `moirai-iter/examples/nested_stress.rs` workload now runs outside libtest
  with a 300-pass default and a 20,000-pass ceiling; quiet and twenty-burner
  release runs each completed **300/300** passes with exact-value checks. The
  non-libtest shape still did not reproduce the crash.
- **Miri job-storage check (2026-09-17).** Nightly Miri ran all 13
...
- **Miri nested-scope check (2026-09-16).** Nightly Miri ran
  `scheduler_scope_nested_saturation_completes`; all three worker-count
  cases completed with the exact `32 × 16` increment oracle (**1/1** test).
  Miri found no race or invalid access in this nested scope shape; the rare
  libtest-only crash still has no reproducing execution under a checker.
- **Miri recursive-scope check (2026-09-16).** Nightly Miri ran
  `scheduler_scope_recursive_fork_join_is_sound`; the `200,000`-element
...
- Status: review; priority: correctness; integrator: root; delivery: commit
  `1b8046dd`; last-update: 2026-09-16.
- Outcome: ordering sleeps in the channel, task, executor, iterator, PAL,
  synchronization, and integration tests now use readiness/release channels,
  barriers, waker publication, cooperative yields, deterministic pending polls,
  or return events. No modified test compares scheduler elapsed time.
- Verification: locked filtered nextest runs 738/738 tests (6 skipped); workspace
  all-target Clippy with `-D warnings`, format, diff, and lockfile checks pass.
- Residuals: timer-under-test sleeps, ignored timing instruments, production
  retry/polling/backoff/timer sleeps, and examples/docs remain with their
  behavior or measurement contracts; no test uses a duration to order threads.

<a id="MOI-FS-CONFINED-2026-09-09"></a>
## MOI-FS-CONFINED-2026-09-09 — Open native files beneath a directory handle [minor] [arch]

- Status: done; priority: P0; integrator: root; delivery: [Moirai PR #303](https://github.com/ryancinsight/Moirai/pull/303) (`b1fe7ad5`); last-update: 2026-09-09.
- Outcome: `moirai-pal` owns the Unix `openat` and Windows relative `NtCreateFile` walk, rejects traversal and link components, and reports unsupported on WebAssembly; RITK consumes the handle for DICOM reads. Decision: [ADR 0053](adr/0053-root-confined-file-opening.md).
- Verification: native PAL tests, warning-denied Clippy, Unix-target Clippy, WebAssembly check, doctests, and rustdoc passed.
...
- Status: done; priority: P0; integrator: root; delivery: `9a0af648`; last-update: 2026-09-09.
- Outcome: fixed 16-slot allocation-free admission, typed overflow, unlocked snapshots, caller migration and deterministic owner-thread reclamation tests; the full workspace gate passes 1,062/1,062 tests.
- Decision: [ADR 0051](adr/0051-bounded-idle-hook-admission.md); the `register_idle_hook` return change is classified major and remains unreleased.
...
- Acceptance: selected file handles preserve validated metadata, bounded reads
  are input-sensitive and seekable, invalid positions and oversized requests
...
- Evidence: `cargo check --workspace --locked`, `cargo test -p moirai-pal
  --locked` (60 passed, including the finite wait/input wake case) and strict
  package Clippy pass on Windows.
- Increment: `NativeWindow::wait_events` accepts a finite timeout capped at 30
...
- Class: [arch] [minor]; status: done; integrator: root; last-update: 2026-09-07.
- Driver: [Metis crypto](../../metis/backlog.md#METIS-CRYPTO-001); decision: [ADR 0044](adr/0044-standalone-cryptographic-primitives.md).
- Evidence: Moirai `66627b9` merged by PR [#266](https://github.com/ryancinsight/Moirai/pull/266); standalone vectors, native provider checks, no-default WASM and warning-denied Clippy passed; Metis PR [#14](https://github.com/ryancinsight/metis/pull/14) merged at `2258266` with no duplicate implementation.
...
- Class: [major] [arch]; status: done; integrator: root; last-update: 2026-09-06.
...
- [x] [patch] MOI-GPU-SAFE-001: harden GPU buffer range boundaries. Centralize
  ...
  release builds. Added value-semantic boundary tests. Evidence: offline
...
- **Delivered:** `0a9ef5e`, `7e6ef4e`, `63e3289`; PR #231; merge
  `d352ec1`; lease: none.
- **Outcome:** both map paths share one panic-safe output owner; gross bytes for
  the warmed 2,097,153-value census fall from 16,777,624 to 16,777,424 with no
  detected Criterion regression. Independent review and all exact-head
...
- **Scope / non-goals:** one crate-private retained-slot primitive under
  `moirai-iter/src/stream/`, ordered consumers in `execution/base.rs`,
  `async_iter/parallel.rs`, and `ConcurrentStreamExt::concurrent_map_ordered`,
  focused allocation/value/pending/drop tests, retained Criterion and contract
  coverage, CHANGELOG, and PM state. The public unordered completion-order
  stream remains on futures-util unless a candidate preserves wake completion
  order exactly. No public API, scheduler, AsyncIterator trait/ADR-018,
...
- **Acceptance:** entry censuses cover ready and one-pending-poll public map and
  ...
  Criterion row by 5% or more. Any pin projection stays behind a safe private
  API with SAFETY proofs, Miri coverage, panic/drop tests, and warning-denied
  cross-target checks.
...
- **Candidate evidence:** a contiguous-slab scan candidate reduced ready and
  ...
  bound versus 180.65 us for futures-util and was rejected. A first segmented
  fix-forward then exposed quadratic initial vacancy discovery and measured
  ...
  2,232. The eight-byte intrusive link per retained slot changes bytes, not
  allocation count. Final Criterion median estimates (95% CI) are 25.326 us
  ...
  and the one-slot/two-generation stale-wake Loom
...
- **Evidence:** the warmed public 1,024-item ready-future map moves from 1,118
  ...
  attribution target, not a removed-allocation claim in this item. Exact-source
  gates pass: focused allocation and contract tests; debug and release
  all-feature suites 233/233 with two configured skips each; warning-denied
  all-target/all-feature Clippy, Rustdoc, and AArch64 Windows all-target check;
  4/4 doctests; benchmark smoke; formatting/diff checks; and
  cargo-semver-checks 223/223 under patch.
...
- **Evidence:** at exact entry source `8960e3b`, warmed zero-copy construction
  ...
  cargo-semver-checks 223/223 under patch pass. Independent review of `871a1c9`
  found an
  ...
  Exact corrected source `66bc88b` passed independent static re-review. Every
  repository check on PR #224 passed; the external non-required
  `recurseml/analysis` service errored.
...
- **Delivered:** reviewed head `b8a5325`, PR #210 merge `f82d83d`; all required
  repository checks passed. The bounded workflow executes all 16 committed
...
- **Delivered:** source `d94ef46`, PM `c9eacef`, PR #207, merge
  `c35205b067e0428904bcebdc40862126106c68dd`. Independent review and every
  repository check are GREEN; the non-required external analysis service errored.
...
- **Evidence:** the exact probe records 1,572,864 direct retained bytes at 128 versus 3,145,728 at 256 across 24 workers, overlapping controlled warm confidence intervals, zero warm fan-out allocations, and zero retained bytes after teardown. Provider Nextest passes 328/328; host, AArch64, documentation, SemVer, and all hosted gates pass.
...
- **Evidence**: PR #174 merged as `f162c59`; generation-matched cleanup rejects stale `POLLNVAL` events after raw-socket reuse. Warning-denied Windows check/Clippy, PAL+TLS Nextest 25/25, and workspace Nextest 835/835 in 11.096 seconds pass.
...
- **Outcome**: committed fixtures now verify trusted, wrong-hostname, untrusted, and expired TLS handshakes by Rustls error variant without `rcgen`'s native certificate-generation test graph.
...
- **Outcome**: native files implement exact `AsyncReadAt` and `AsyncLength`; Unix uses cursor-independent reads, Windows synchronizes only positioned cursor restoration, and the PAL file-system module is split into bounded leaves.
...
- **Resolution**: forward the facade value through the `numa` feature closure
  ...
  direct `ThreadScheduler` construction retains topology-aware defaults;
...
- Status: done (2026-08-24, Part IV complete through Moirai PR #160:
  chapters 1-6 written against current sources - transports #155,
  payloads #156, safe channels #157, routes #158, router #159, remote
  tasks #160; no placeholders or dead links; mdbook builds clean)
...
- Status: done; implementation: `aa65267f`; deliveries: [PR #387](https://github.com/ryancinsight/Moirai/pull/387) (`617b55a99360a2d50ed0007ea918b8ba20305245`), [PR #388](https://github.com/ryancinsight/Moirai/pull/388) (`c59cd6ccb936d7aaf53cd79ae90b7a016974e0b5`); last-update: 2026-09-17.
...
- Delivery correction: the reported PR #387 head is `cf4affe86e4fa57ff40db26548bebbfd249896de`; a manually reconstructed invalid SHA caused the first consumer fetch to fail, and PR #387 merged administratively while fresh hosted checks were running outside the unavailable-or-starved-CI exception. Run [35176859540](https://github.com/ryancinsight/Moirai/actions/runs/35176859540) exposed PR #386's stale benchmark contract; PR #388 repaired it at `a3a0ae1400a400046b62d9bf0f40b150d7e02141`, and hosted run [35179224537](https://github.com/ryancinsight/Moirai/actions/runs/35179224537) passed every applicable job.

<a id="MOI-WINDOWS-SOCKET-LIFETIME-2026-09-17"></a>
## MOI-WINDOWS-SOCKET-LIFETIME-2026-09-17 — Retain polled socket owners [patch]
- Status: review; priority: P0; integrator: Codex `/root`; last-update: 2026-09-17T01:57:10-04:00.
- Outcome: Windows poll snapshots retain PAL TCP/UDP owners through `WSAPoll`; future cancellation targets the originating reactor and exact per-interest waiter. Genuine driver failures remain terminal.
- Scope: PAL network/reactor code, async cancellation and executor regressions, affected benchmark contracts, [ADR 0014](adr/0014-reactor-bound-async-i-o-and-readiness-integration.md), and checklist. No version bump.
- Acceptance: real-socket retirement and mixed snapshots, replacement-safe cancellation, reentrant destructor safety, exact later payload, captured stale-waker safety, and configured workspace/Windows release/documentation/SemVer gates.
- Verification: fingerprint `3fbca29b0ba17b6923244a6e78d8ac1155876f19eb8b4e09ae1123e21787d2ff` at `1688fe8b`; fmt and workspace all-feature/all-target Clippy pass; nextest `f6b594a2` passes 1,129 tests (ten skips), PAL all-feature release `31439d71` passes 99 (one skip), doctests pass 26 (one ignored), Rustdoc passes with warnings denied. Commands/logs: downstream `output/moirai-socket-final/verification.json`. Subsequent edits only correct these delivery records and add resolved ADR citations; tested code remains byte-identical.
