...
| Non-Clone borrowed source map | `Vec<T>::par_iter().map(...).sum_reassociated()` over borrowed non-`Clone` values | `test_non_clone_parallel_ref_iterator_maps_borrowed_values` and `iterator_adapter_non_clone_ref_map` against Rayon | Covered bounded source boundary |
...
| Standard numeric/order reducers | `sum`, `product`, `min`, `max`, `min_by`, `max_by`, `min_by_key`, `max_by_key` | Standard `sum` and `product` invoke their trait once over the complete logical stream; custom-accumulator tests reject stronger output-self bounds and reassociation. Comparator/key tests and ordered-reducer benchmark rows cover the remaining methods. | Covered subset; `sum` and `product` are whole-stream by contract |
| Reassociated numeric reducers | `sum_reassociated`, `product_reassociated` | Batch-sensitive tests distinguish partial-output grouping from the standard trait contract; primitive and empty-stream tests cover both operations. `iterator_adapter_terminal_reducers`, `iterator_adapter_non_clone_ref_map`, and `parallel_iterator_regression` opt into the reassociated sum after asserting equivalent primitive results. | Opt-in deterministic shard-fold subset |
...
Completed: `sum`, `product`, `min`, and `max` are implemented as terminal `ParallelIterator` methods with value-semantic tests for non-empty and empty streams. Standard `sum` and `product` invoke `Sum<Item>` and `Product<Item>` once over the complete logical stream. The explicit `sum_reassociated` and `product_reassociated` methods expose deterministic partial-output grouping under stronger output-self trait bounds; the former retains the benchmarked shard-fold path after primitive-value assertions.
...
The 2026-06-02 expanded focused rerun produced the table below. The two paths
affected by ISSUE-210 were refreshed on 2026-07-13 after removing leaked input
allocations; those six rows are the current same-run ranges.
...
| `parallel_iterator_collect_into_existing_sizes` / 1,024 | 305.78-313.45 ns | 37.548-37.887 us | Moirai ahead |
| `parallel_iterator_collect_into_existing_sizes` / 32,768 | 34.716-35.626 us | 112.90-118.62 us | Moirai ahead |
| `parallel_iterator_collect_into_existing_sizes` / 131,072 | 686.26-700.82 us | 802.51-812.70 us | Moirai ahead |
...
| `parallel_iterator_indexed_step_interleave_sizes` / 1,024 | 918.37-928.49 ns | 38.795-40.801 us | Moirai ahead |
| `parallel_iterator_indexed_step_interleave_sizes` / 32,768 | 51.905-52.016 us | 123.47-129.30 us | Moirai ahead |
| `parallel_iterator_indexed_step_interleave_sizes` / 131,072 | 750.84-780.64 us | 791.03-801.27 us | Moirai ahead |
