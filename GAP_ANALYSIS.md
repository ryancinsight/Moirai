...
- `moirai-iter::ThreadPool` queues `ErasedThreadJob` values with monomorphized run/drop functions instead of `Box<dyn FnOnce>` queue items.
...
- Full drop-in compatibility with every Tokio I/O type and every Rayon `ParallelIterator` adapter is an ecosystem extension goal, not an active scheduler comparison gap. The current audit covers a Moirai-owned file facade read benchmark against Tokio and Moirai-owned TCP/UDP network facade value tests.
...
- Async file facade update: `moirai_async::fs::write`, `moirai_async::fs::append`, and `moirai_async::fs::copy` now delegate to PAL platform operations instead of constructing the higher-level facade for write/append or allocating a 64 KiB user-space transfer buffer for copy. `async_fs_comparison` now covers `async_fs_read_to_end`, `async_fs_write_file`, `async_fs_append_file`, and `async_fs_copy_file` against Tokio with byte equality assertions. The latest read row measures Moirai at 39.127-45.710 µs versus Tokio at 96.964-100.34 µs. The latest write row measures Moirai at 2.8650-3.4698 ms versus Tokio at 2.5939-3.2074 ms. The latest append row measures Moirai at 272.59-291.93 µs versus Tokio at 190.29-320.18 µs. The latest copy row measures Moirai at 536.26-604.18 µs versus Tokio at 541.41-716.30 µs.
