...
- `moirai-transport/scheduler-routes` now consumes `SchedulerRoute` values through `RouteAddressBook`, `RoutedArchivedSender<P>`, and `RoutedArchivedReceiver<P>`, binding route metadata to transport-owned archive bytes for local roundtrips and to remote endpoint metadata for known server routes. OS process execution, server transport execution, and Mnemosyne allocator handoff remain open.
- `NetworkTransport` now transfers remote payload bytes over a bounded length-prefixed TCP frame and `TransportManager` routes `Address::Remote` through that real byte path. Remote task envelopes/results, process lifecycle, production server backpressure, and Mnemosyne allocator handoff remain open.
