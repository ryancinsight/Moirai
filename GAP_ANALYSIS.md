...
- Async file facade update: `moirai_async::fs::copy` now delegates to the PAL platform copy operation instead of allocating a 64 KiB user-space transfer buffer. `async_fs_comparison` now covers both `async_fs_read_to_end` and `async_fs_copy_file` against Tokio with byte equality assertions. The latest read row measures Moirai at 39.127-45.710 µs versus Tokio at 96.964-100.34 µs. The latest copy row measures Moirai at 536.26-604.18 µs versus Tokio at 541.41-716.30 µs.
