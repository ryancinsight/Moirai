...
- Async file facade update: `moirai_async::fs::write` and `moirai_async::fs::copy` now delegate to PAL platform operations instead of constructing the higher-level facade for write or allocating a 64 KiB user-space transfer buffer for copy. `async_fs_comparison` now covers `async_fs_read_to_end`, `async_fs_write_file`, and `async_fs_copy_file` against Tokio with byte equality assertions. The latest read row measures Moirai at 39.127-45.710 µs versus Tokio at 96.964-100.34 µs. The latest write row measures Moirai at 2.8650-3.4698 ms versus Tokio at 2.5939-3.2074 ms. The latest copy row measures Moirai at 536.26-604.18 µs versus Tokio at 541.41-716.30 µs.
