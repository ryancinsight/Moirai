...
impl<T> SendConstPtr<T> {
	unsafe fn as_ptr(&self) -> *const T { self.0 }
}
...
impl<'a, T> WindowIterator<'a, T> {
	...
	pub fn new(data: &'a [T], window_size: usize, stride: usize) -> Self {
		...
		Self { data, window_size, stride, position: 0 }
	}
...
impl<'a, T> Iterator for WindowIterator<'a, T> {
	...
	fn size_hint(&self) -> (usize, Option<usize>) {
		if self.position >= self.data.len() { return (0, Some(0)); }
...
impl<'a, T: Sync> ZeroCopyParallelIter<'a, T> {
		...
		T: Clone + Send,
	{
		...
		if self.data.len() == 1 { return Some(self.data[0].clone()); }
		...
		while current_results.len() > 1 {
			current_results = std::thread::scope(|scope| {
				...
				handles.into_iter().flat_map(|h| h.join().ok().unwrap_or_default()).collect()
			});
		}
