...
mod tests {
    ...
    use crate::{DeviceFeature, FunctionGpuTask, GpuTask};
    ...
    fn configured_task_executes_and_reports_scheduler_cost() -> Result<()> {
        let task = GpuTaskBuilder::new(FunctionGpuTask::new(|device: &HostDevice| {
    ...
    fn configured_task_rejects_unavailable_features_before_execution() {
        let task = GpuTaskBuilder::new(FunctionGpuTask::new(|_device: &HostDevice| {
            Ok::<_, hephaestus_core::HephaestusError>(1_u32)
        }))
