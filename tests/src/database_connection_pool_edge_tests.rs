...
impl DatabaseConnection {
    ...
    fn execute_query(&self, query: &str, simulate_failure: bool) -> Result<QueryResult, DatabaseError> {
        if simulate_failure {
...
impl DatabaseConnectionPool {
    ...
    fn start_maintenance_tasks(&self) -> Result<(), String> {
        if let Some(runtime) = &self.runtime {
            ...
            let handle = runtime.spawn_fn_with_priority(move || {
                loop {
    ...
    fn get_statistics(&self) -> PoolStatistics {
        let available_count = self.available_connections.lock()
            .map(|conns| conns.len()).unwrap_or(0);
        let borrowed_count = self.borrowed_connections.lock()
            .map(|conns| conns.len()).unwrap_or(0);
    ...
    fn reap_connections(&self) {
        ...
        if let Ok(mut available) = self.available_connections.lock() {
            ...
            if available.len() != initial_count {
                println!("Reaped {} idle/old connections", initial_count - available.len());
            }
        }
    }
}
...
impl DeadlockDetector {
    fn detect_and_resolve_deadlocks(&self) {
        if let Ok(borrowed) = self.borrowed_connections.lock() {
            ...
            let stuck_connections: Vec<_> = borrowed.iter()
                .filter(|(_, (_, borrowed_time))| {
                    _now.duration_since(*borrowed_time).as_millis() > self.connection_timeout_ms as u128
                })
                .map(|(id, _)| *id)
                .collect();

            if !stuck_connections.is_empty() {
                println!("DEADLOCK DETECTION: Found {} stuck connections", stuck_connections.len());
...
impl<'a> PooledConnection<'a> {
    ...
    fn execute_query_with_failure(&self, query: &str, simulate_failure: bool) -> Result<QueryResult, DatabaseError> {
        if let Some(conn) = &self.connection {
...
impl DatabaseWorkloadSimulator {
    ...
    fn simulate_normal_workload(&self, operation_count: usize) -> Result<WorkloadResults, String> {
        ...
        for i in 0..operation_count {
            ...
            let handle = self.runtime.spawn_fn_with_priority(move || {
                ...
                match pool.get_connection(5000) { // 5 second timeout
                    Ok(conn) => {
                        let query = format!("SELECT * FROM users WHERE id = {}", i);
                        match conn.execute_query(&query) {
                            Ok(_result) => {
                                successful.fetch_add(1, Ordering::Relaxed);
                                query_time.fetch_add(query_start.elapsed().as_millis() as u64, Ordering::Relaxed);
                            }
                            Err(_) => {
        ...
        Ok(WorkloadResults {
            ...
            total_time: elapsed,
            average_query_time_ms: if successful_count > 0 { total_time / successful_count as u64 } else { 0 },
        })
    }

    /// Simulate connection exhaustion scenario
    fn simulate_connection_exhaustion(&self, concurrent_requests: usize) -> Result<WorkloadResults, String> {
        let start_time = Instant::now();
        let successful_operations = Arc::new(AtomicUsize::new(0));
        let failed_operations = Arc::new(AtomicUsize::new(0));
        ...
        for i in 0..concurrent_requests {
            ...
            let handle = self.runtime.spawn_fn_with_priority(move || {
                match pool.get_connection(1000) { // Short timeout to trigger exhaustion
                    Ok(conn) => {
                        // Hold connection for a while to create contention
                        thread::sleep(Duration::from_millis(100));
                        
                        let query = format!("SELECT COUNT(*) FROM orders WHERE user_id = {}", i);
                        match conn.execute_query(&query) {
    ...
    fn simulate_failure_scenarios(&self, operation_count: usize) -> Result<WorkloadResults, String> {
        let start_time = Instant::now();
        let successful_operations = Arc::new(AtomicUsize::new(0));
        let failed_operations = Arc::new(AtomicUsize::new(0));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match simulator.simulate_normal_workload(100) {
        Ok(results) => {
            println!("  ├─ Successful: {}/{}", results.successful_operations, results.total_operations);
    ...
    println!("  └─ 1000 rapid cycles completed in {:?}", start_time.elapsed());
    ...
    println!("  ├─ Connections destroyed: {}", stats.connections_destroyed);
    ...
    println!("  ├─ Pool efficiency: {:.2}%", 
             ((stats.connections_created - stats.connections_destroyed) as f64 / stats.connections_created as f64) * 100.0);
    ...
    println!("  ├─ Operations under pressure: {} successful, {} failed", 
             pressure_success, pressure_failed);
    println!("  └─ Pressure test completed in {:?}", pressure_start.elapsed());
